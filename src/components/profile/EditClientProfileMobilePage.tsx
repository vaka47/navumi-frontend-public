...
export default function EditClientProfileMobilePage({
                                                        open,
                                                        onClose,
                                                        initialData,
    ...
    // скрыть нижний бар + залочить scroll body
    ...
    // ошибки + автопрокрутка к верху формы/алерту
    ...
    // быстрый системный диалог (создаём input на лету)
    ...
    // дозапрос окна смены имени (только в открытом состоянии)
    useEffect(() => {
        if (!open) return;
    ...
    return (
        <div className="fixed inset-0 z-[10000] bg-white flex flex-col">
            ...
            <div className="h-[56px] flex items-center justify-between px-4 border-b border-gray-200">
                ...
                <button onClick={onClose} aria-label="Закрыть" className="p-1 -mr-1 text-2xl leading-none">
                    ✕
                </button>
            </div>
                        ...
                        <div
                            ...
                            onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ' ') {
                                    e.preventDefault();
                                    if (!croppedAvatar && !originalAvatar && !serverAvatarUrl) openFileDialogFast();
                                    else handleAvatarClick();
                                }
                            }}
                ...
                <div className="space-y-0">
                    ...
                    {canChangeUsername === true && username !== initialData.username && usernameTouched && (
                        <div className="-mt-2 space-y-1 px-1">
                            {usernameStatus === 'available' && <p className="m-0 text-sm text-green-600">Имя свободно</p>}
                            {usernameStatus === 'taken' && <p className="m-0 text-sm text-red-600">Имя занято ❌</p>}
                            {usernameStatus === 'invalid' && (
                                <p className="m-0 text-sm text-red-600">Допустимы только латиница, цифры и подчёркивание.</p>
                            )}
                        </div>
                    )}
                    ...
                    {canChangeUsername === null && (
                        <p className="-mt-2 px-1 m-0 text-sm text-gray-400">Проверяем возможность смены имени…</p>
                    )}
                </div>
          ...
          />
            ...
            {/* Подтверждение удаления аватарки */}
            {confirmDeleteOpen && (
                <div className="fixed inset-0 z-[11000]">
                    ...
                    <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-sm w-full bg-white rounded-xl p-6 shadow-lg">
                        ...
                        <p className="text-sm text-gray-600 mb-4">Вернуть его можно будет только повторно загрузив файл.</p>
